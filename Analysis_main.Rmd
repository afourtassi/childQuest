...
ggplot(data_turns_sum,  aes(age2, Turns_sum, col= Role))+
  ...
  geom_smooth(method = lm, se = F, alpha=0.15,aes(fill=Role))+
   theme_few()+
  ...
  facet_wrap(~child, scales= "free_y")+
...
```{r}
child_dep <- child %>%
  mutate(Response = if_else(`PAR-Response` == "No-response" | `PAR-Response` == "No-label" | `PAR-Response` == "Non-answer", "non-contingent", "contingent")) %>%
...
caregiver_dep <- caregiver %>%
  mutate(Response = if_else(`CHI-Response` == "No-response" | `CHI-Response` == "No-label" | `CHI-Response` == "Non-answer", "non-contingent", "contingent")) %>%
...
all_dep <- all_dep %>%
  bind_rows(all_dep_all)
  

all_dep$Role = factor(all_dep$Role, levels = c("Child", "Caregiver"))

all_dep$child = factor(all_dep$child, levels = c("Adrien", "Anae", "Madeleine", "Theophile", "All"))
...
ggplot(all_dep,  aes(Response, Feedback))+
  ...
  stat_summary(fun.data=mean_cl_normal,position=position_dodge(width=1),geom="pointrange",size=0.3) + 
  ...
  ggtitle("Follow-up by Response type")+
      
  xlab("Response type") +ylab("% Contingent follow-up") +
  facet_grid(Role~child, scales= "free_y") +
...
followups <- data_processed %>%
  ...
  select(-label, -label_type) %>%
  mutate(type = "Follou-up")

contingency_all <- responses %>%
  bind_rows(followups)
  
contingency_all$type = factor(contingency_all$type, levels = c("Response", "Follou-up"))
...
ggplot(contingency_all,  aes(age2, contingency, col=Role))+
  ...
  xlab("Child age (years)") +ylab("% contigent given opportunity")  +
  facet_grid(type~child)
...
questions_all <- questions %>%
  bind_rows(questions_total)

questions_all$child = factor(questions_all$child, levels = c("Adrien", "Anae", "Madeleine", "Theophile", "All"))

ggplot(questions_all,  aes(age2, quest_per_file))+
  ...
  ggtitle("Questions")+
  xlab("Child age (years)") +ylab("Questions per transcript")  +
  facet_grid(Role~child,  scales= "free_y")
```
...
QRF_combined <- responses %>%
  full_join(followups)
  
QRF_combined[is.na(QRF_combined)] <- 0

QRF_combined <- QRF_combined %>%
  gather(key, value, Responses, Followups)

QRF_combined$key = factor(QRF_combined$key, levels = c("Responses", "Followups"))

QRF_combined_all <- QRF_combined %>%
  select(-child) %>%
  mutate(child ="All")

QRF_combined <- QRF_combined %>%
  bind_rows(QRF_combined_all)

QRF_combined$child = factor(QRF_combined$child, levels = c("Adrien", "Anae", "Madeleine", "Theophile", "All"))
...
ggplot(QRF_combined %>% filter(key == "Responses"),  aes(age2, 100*value))+
  geom_point(size=2 ,alpha= 0.2)+
  geom_smooth(method = lm, se = T, alpha=0.2)+
   theme_few()+
  theme(aspect.ratio = 0.7, 
  ...
  ggtitle("Responses")+
  xlab("Child age (years)") +ylab("% contigent per transcript")  +
  facet_grid(Role~child,  scales= "free_y") +
  coord_cartesian(ylim = c(0, 100)) 



ggplot(QRF_combined %>% filter(key == "Followups"),  aes(age2, 100*value))+
  geom_point(size=2 ,alpha= 0.2)+
  geom_smooth(method = lm, se = T, alpha=0.2)+
   theme_few()+
  theme(aspect.ratio = 0.7, 

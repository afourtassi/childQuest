...
(are we really have a good estimate of all non-answers?)
...
```
What are Contingent responses made of ?

```{r}

response_subtypes <- data_processed %>%
  filter(Role != "OTHER") %>%
  filter(label == 'CHI-Response' | label == 'PAR-Response') %>%
  mutate(Role_annot = unlist(strsplit(label, '-'))[1] ) %>%
  filter(label_type =="Answer" | label_type =="Non-answer") %>%
  group_by(child, file, Role_annot, label, label_type, label_subtype) %>%
  summarise(subtype_per_file = n()) %>%
  ungroup() %>%
  group_by(child, file, Role_annot, label, label_type) %>%
  mutate(type_total = sum(subtype_per_file)) %>%
  ungroup() %>%
  dplyr::rename(Role = Role_annot) %>%
  rowwise() %>%
  mutate(age2 = convertDate(file)) %>%
  mutate(subtype_per_type = subtype_per_file/type_total)

ggplot(response_subtypes %>% filter(label_type =='Answer'),  
       aes(age2, subtype_per_type , col= label_subtype))+
  geom_point()+
  geom_smooth(method = lm, se = FALSE)+
  theme_few()+
  theme(aspect.ratio = 0.7, 
  ...
  xlab("Age") +ylab("% out of all contigent responses") +
  facet_grid(.~Role, scales= "free_y")
...
follow_types <- data_processed %>%
  ...
  summarise(type_per_file = n()) %>%
  ungroup() %>%
  mutate(res_broad = if_else(label_type == "No-followup" | label_type == "No-followup"   "No-response", "Response")) %>%
  group_by(child, file, Role_annot, label) %>%
  mutate(res_total = sum(type_per_file)) %>%
  ungroup() %>%
  group_by(child, file, Role_annot, res_broad) %>%
